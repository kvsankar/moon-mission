...
for (const [tier, width] of [["low", 1024], ["medium", 2048]]) {
    ...
    const normalData = buildPhysicalMoonNormalData({ heightData, width, height, physicalHeightScale: MOON_HEIGHT_SCALE, slopeBoost: 1 });
    ...
    const file = `images/moon/terrain-${tier}-v1.moon.gz`;
...
await fs.writeFile(path.join(root, "images/moon/terrain-v1-provenance.json"), JSON.stringify({
    ...
    normals: "buildPhysicalMoonNormalData; slopeBoost=1; same function used by High worker",
